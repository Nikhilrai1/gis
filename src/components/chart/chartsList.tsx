...
import { useDeleteChartMutation, useGetAllChartsQuery } from "@/redux/features/chart/chartApi";
...
const ChartsList = () => {
    ...
    const { data, isLoading: getLoading } = useGetAllChartsQuery({
    ...
    const [deleteChart, { isLoading: deleteLoading, isSuccess: deleteSuccess }] = useDeleteChartMutation();
                        ...
                        <table className="min-w-max w-full">
                            <thead className='bg-theme'>
                                <tr className="bg-primary-blue-900 text-white capitalize text-sm leading-normal">
                                    ...
                                    <th className="py-3 px-6 text-center">{"Chart Type"}</th>
                                    <th className="py-3 px-6 text-center">{"Form"}</th>
                                    <th className="py-3 px-6 text-center">{"Form Field"}</th>
                            ...
                            <tbody className="text-gray-600 text-sm font-light">
                                {(data?.results && data?.results?.length > 0) && data?.results?.map((el, i) => (
                                    <tr key={i} className="border-b border-gray-200 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                                        ...
                                        <td className="py-3 px-6 text-left">
                                            <div className="flex items-center justify-center">
                                                <span className="font-medium">{el?.chart}</span>
                                            </div>
                                        </td>

                                        <td className="py-3 px-6 text-left">
                                            <div className="flex items-center justify-center">
                                                <span className="font-medium">{el?.form}</span>
                                            </div>
                                        </td>

                                        <td className="py-3 px-6 text-left">
                                            <div className="flex items-center justify-center">
                                                <span className="font-medium">{el?.form_field}</span>
                                            </div>
                                        </td>

                                        <td className="py-3 px-6 text-left">
                                            <div className="flex items-center justify-center">
                                                <span className="font-medium">{el?.operation}</span>
                                            </div>
                                        </td>
